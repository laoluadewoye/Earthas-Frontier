...
use crate::utils::result::{EFResult};
...
impl EFByteVecCompatible for EFURIAuthority where Self: Sized {
    fn to_byte_vec(&self) -> EFResult<Vec<u8>> {
        match self {
            EFURIAuthority::Global => EFResult::new_with_init(
                Some(vec![0u8]),
                true,
                "to_byte_vec", 
                "EFURIAuthority::Global", 
                "Created byte vector for global URI authority."
            ),
            EFURIAuthority::Local => EFResult::new_with_init(
                Some(vec![1u8]),
                true,
                "to_byte_vec", 
                "EFURIAuthority::Local", 
                "Created byte vector for local URI authority."
            ),
            EFURIAuthority::Connection(enum_str) => EFResult::new_with_init(
                Some(vec![1u8]),
                true,
                "to_byte_vec", 
                "EFURIAuthority::Local", 
